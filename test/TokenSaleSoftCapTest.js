...
contract("Soft Cap Test", function(accounts) {
    ...
    it("Register hunter from known address", async function() {
        result = await contract.registerHunter({from: accounts[2]});
        result = await contract.isHunter(accounts[2]);
        assert.strictEqual(result, true, "Not a hunter");
        result = await contract.getReferralCode({from: accounts[2]});
        assert.notStrictEqual(result, "0x000000", "No code generated");
        code = result;
        result = await contract.isReferralCodeValid(code, {from: accounts[1]});
        assert.strictEqual(result, true, "Invalid code");
    });

    it("Register hunter from unknown address", async function() {
        result = await contract.registerHunter({from: accounts[3]});
        result = await contract.isHunter(accounts[3]);
        assert.strictEqual(result, true, "Not a hunter");
        result = await contract.getReferralCode({from: accounts[3]});
        assert.notStrictEqual(result, "0x000000", "No code generated");
        code = result;
        result = await contract.isReferralCodeValid(code, {from: accounts[1]});
        assert.strictEqual(result, true, "Invalid code");
